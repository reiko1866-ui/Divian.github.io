(function () {
  ...
  function focusDest(lngLat) {
    ...
    try {
      state.map.easeTo({
        ...
        zoom: Math.max(Number(state.map.getZoom()) || 0, 14.6),
  ...
  function offlineMapUrl() {
    try {
      return new URL("./map/hungary.pmtiles", document.baseURI).href;
    } catch (_e) {
      return "./map/hungary.pmtiles";
    }
  }

  function offlineMaxZoom() {
    const header = window.__pmtilesHeader;
    const z = header && Number(header.maxZoom);
    return Number.isFinite(z) && z > 0 ? z : MAP_SOURCE_MAXZOOM;
  }
  ...
  async function loadPmtilesArchive() {
    ...
    try {
      setStatus("Magyarország térkép betöltése…");
      ...
      try {
        window.__pmtilesHeader = await tiles.getHeader();
      } catch (_h) {}
